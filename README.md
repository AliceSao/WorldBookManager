...
| 仓库 | 内容 |
|---|---|
| **WorldBookManager**（本仓库） | 全部源代码 + 文档 + Python CLI |
| **ST-WBM-UI** | 仅运行时文件（前端扩展，直接安装到 ST） |
| **ST-WBM-Server** | 仅运行时文件（后端插件，直接安装到 ST） |
...
1. 在 SillyTavern 扩展面板中点击 **📖 打开管理面板**（通过 iframe 弹窗打开）
...
| 文档 | 说明 |
|---|---|
| [安装指南](docs/st_wbm/INSTALL.md) | ST-WBM 安装步骤（后端 + 前端） |
...
- **ST 扩展**：原生 JavaScript（面板注入 + iframe 弹窗）
