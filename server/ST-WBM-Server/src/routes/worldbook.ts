...
export function registerWorldbookRoutes(router: Router): void {
  ...
  router.get("/csrf-token", (req: Request, res: Response) => {
    try {
      const token = typeof (req as unknown as { csrfToken?: () => string }).工作世界书Token === "function"
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.get("/worldbooks", async (req: Request, res: Response) => {
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.get("/worldbooks/:name", async (req: Request, res: Response) => {
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.put("/worldbooks/:name", async (req: Request, res: Response) => {
    const { name } = req.params;
    const { entries, user = "default-user" } = req.body as {
      entries: RawEntry[];
      user?: string;
    };
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.post("/worldbooks", async (req: Request, res: Response) => {
    const { name, entries = [], user = "default-user" } = req.body as {
      name: string;
      entries?: RawEntry[];
      user?: string;
    };
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.delete("/worldbooks/:name", async (req: Request, res: Response) => {
    const { name } = req.params;
    const user = String(req.query.user || "default-user");
    try {
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.get("/worldbooks/:name/export", async (req: Request, res: Response) => {
    const { name } = req.params;
    const user = String(req.query.user || "default-user");
    try {
      const entries = await readWorldbook(name, user);
  ...
  // ────────────────────────────────────────────────────────────────────────
  router.post("/worldbooks/:name/import", async (req: Request, res: Response) => {
    const { name } = req.params;
    const { user = "default-user", entries: rawEntries } = req.body as {
      user?: string;
