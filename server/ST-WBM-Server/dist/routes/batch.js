...
export function registerBatchRoutes(router) {
    // ──────────────── 激活策略 ────────────────
    ...
    router.post("/worldbooks/:name/batch/depth", async (req, res) => {
        ...
        if (typeof depth !== "number")
            return fail(res, "参数错误：depth 须为整数");
        try {
    ...
    router.post("/worldbooks/:name/batch/order", async (req, res) => {
        ...
        if (typeof order !== "number")
            return fail(res, "参数错误：order 须为整数");
        try {
    ...
    router.post("/worldbooks/:name/batch/name", async (req, res) => {
        ...
        if (typeof title !== "string")
            return fail(res, "参数错误：title 须为字符串");
        try {
    ...
    router.post("/worldbooks/:name/batch/keys/set", async (req, res) => {
        ...
        if (!Array.isArray(keys))
            return fail(res, "参数错误：keys 须为字符串数组");
        try {
    ...
    router.post("/worldbooks/:name/batch/group-weight", async (req, res) => {
        ...
        if (typeof groupWeight !== "number")
            return fail(res, "参数错误：groupWeight 须为整数");
        try {
    ...
    router.post("/worldbooks/:name/batch/keys/replace", async (req, res) => {
        ...
        if (typeof findKey !== "string" || !findKey)
            return fail(res, "参数错误：findKey 须为非空字符串");
        try {
            await withBatch(res, name, user, uids, (e, u) => batchReplaceKey(e, u, findKey, replaceWith), replaceWith
    ...
    router.post("/worldbooks/:name/batch/enabled", async (req, res) => {
        ...
        if (typeof enabled !== "boolean")
            return fail(res, "参数错误：enabled 须为 true/false");
        try {
    ...
    router.post("/worldbooks/:name/copy", async (req, res) => {
        const { name } = req.params;
        const { uids, target_worldbook, user = "default-user" } = req.body;
        if (!Array.isArray(uids) || uids.length === 0)
            return fail(res, "参数错误：uids 不能为空");
        if (!target_worldbook)
            return fail(res, "参数错误：target_worldbook 不能为空");
        try {
            const sourceEntries = await readWorldbook(name, user);
            const toCopy = sourceEntries.filter((e) => uids.includes(e.uid));
            if (toCopy.length === 0)
                return fail(res, "指定的 uid 在源世界书中均不存在", 404);
            const targetEntries = await readWorldbook(target_worldbook, user);
            const copies = toCopy.map(({ uid: _uid, displayIndex: _di, ...rest }) => rest);
