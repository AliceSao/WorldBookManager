...
export function registerEntryRoutes(router) {
    ...
    // ────────────────────────────────────────────────────────────────────────
    router.get("/worldbooks/:name/entries", async (req, res) => {
    ...
    // ────────────────────────────────────────────────────────────────────────
    router.post("/worldbooks/:name/entries", async (req, res) => {
        const { name } = req.params;
        const { entries: overridesList = [{}], user = "default-user" } = req.body;
    ...
    // ────────────────────────────────────────────────────────────────────────
    router.put("/worldbooks/:name/entries/:uid", async (req, res) => {
        const { name } = req.params;
        const uid = parseInt(req.params.uid, 10);
        if (isNaN(uid))
            return fail(res, "参数错误：uid 必须是整数");
        const { user = "default-user", ...updates } = req.body;
        try {
            const entries = await readWorldbook(name, user);
            const updated = updateEntry(entries, uid, updates);
            if (!updated)
                return fail(res, `条目 uid=${uid} 不存在`, 404);
    ...
    // ────────────────────────────────────────────────────────────────────────
    router.delete("/worldbooks/:name/entries", async (req, res) => {
        const { name } = req.params;
